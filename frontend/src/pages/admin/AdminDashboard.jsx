...
import {
  ...
  FiTrendingUp, FiPlus, FiArrowRight, FiActivity, FiClock, FiAlertTriangle
} from 'react-icons/fi';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/orders?limit=1000'), // Get more for revenue calc if needed
      ...
      const lowStock = allProducts.filter(p => p.stockQuantity < 10).length;

      setStats({
        totalOrders: ordersRes.data.total || allOrders.length,
        totalProducts: productsRes.data.total || allProducts.length,
